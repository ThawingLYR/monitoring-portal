...
class PlotIsothermDevelopmentDeepestBoreholes(PlotIsothermDevelopmentBoreholesBase):
    def create_figure(self, sensor):
        df_zero, df_zero_deepest = self._prepare_zero_crossing_data(sensor)
        if df_zero_deepest is None:
            return go.Figure()

        years = sorted(df_zero_deepest["year"].dropna().unique())
        ...
        for year, color in zip(years[:-1], greys_rgba):
            grp = df_zero_deepest[df_zero_deepest["year"] == year]
            if grp.empty:
                continue
        ...
        if latest_year is not None:
            latest_grp = df_zero_deepest[df_zero_deepest["year"] == latest_year]
        ...
        fig.update_layout(
            title=dict(
                text="0 °C isotherm development: deepest", x=0.5, xanchor="center"
            ),
            showlegend=True,
            xaxis=dict(
