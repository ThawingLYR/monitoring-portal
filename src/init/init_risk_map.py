...
GEOJSON_PATH_MB = project_root / "map_data_source" / "risk" / "MB_LYR.geojson"
JSON_PATH_MB = project_root / "map_data_source" / "risk" / "bygningstypekode.json"
GEOJSON_PROCESSED_PATH_MB = (
    project_root / "map_data" / "risk" / "MB_LYR_Processed.geojson"
)
LEGEND_PROCESSED_PATH_MB = project_root / "map_data" / "risk" / "mb_map_legend.html"

GEOJSON_PATH_CH = project_root / "map_data_source" / "risk" / "CH_LYR.geojson"
GEOJSON_PROCESSED_PATH_CH = (
    project_root / "map_data" / "risk" / "CH_LYR_Processed.geojson"
)
LEGEND_PROCESSED_PATH_CH = project_root / "map_data" / "risk" / "ch_map_legend.html"


def get_mb_gdf():
    # Get modern buildings processed data
    gdf = load_geojson_into_gdf(GEOJSON_PROCESSED_PATH_MB)
...
def get_mb_legend():
    ...
    legend = load_html_string(LEGEND_PROCESSED_PATH_MB)
...
def get_ch_gdf():
    # Get modern buildings processed data
    gdf = load_geojson_into_gdf(GEOJSON_PROCESSED_PATH_CH)
...
def get_ch_legend():
    # Get modern buildings map legend
    legend = load_html_string(LEGEND_PROCESSED_PATH_CH)

    return legend


def init_risk_geojson():

    try:
        GEOJSON_PROCESSED_PATH_MB.parent.mkdir(parents=True, exist_ok=True)
    ...
    gdf_mb = load_geojson_into_gdf(GEOJSON_PATH_MB)
    config_mb = load_json(JSON_PATH_MB)

    # Drop irrelevant columns
    gdf_mb = gdf_mb.drop(columns=["MB_date", "HVR_date"])

    # Convert column type
    gdf_mb["MB_type"] = gdf_mb["MB_type"].astype("int64")

    # Convert conguration to dataframe and match MB_type to add configuration to GeoPandas GeoDataFrame (gdf)
    df_config_mb = DataFrame(config_mb)
    gdf_mb = gdf_mb.merge(
        df_config_mb.rename(columns={"kodeverdi": "MB_type"})[["MB_type", "navn"]],
        on="MB_type",
        how="left",
    )

    # Save gdf as geojson
    gdf_mb.to_file(GEOJSON_PROCESSED_PATH_MB)

    # Get data from its online source (or for now in map_data_source)
    gdf_ch = load_geojson_into_gdf(GEOJSON_PATH_CH)

    # Drop irrelevant columns
    gdf_ch = gdf_ch.drop(columns=["FID", "CH_date", "HVR_date"])

    # Save gdf as geojson
    gdf_ch.to_file(GEOJSON_PROCESSED_PATH_CH)
    ...
    # Save legend to html
    with LEGEND_PROCESSED_PATH_MB.open("w", encoding="utf-8") as f:
        f.write(legend_html_mb)
    ...
    # Save legend to html
    with LEGEND_PROCESSED_PATH_CH.open("w", encoding="utf-8") as f:
        f.write(legend_html_ch)
